...
var ColumnsSeparatorView = SeparatorView.inherit({
  ...
  moveByX: function moveByX(outerX) {
    ...
    if ($element) {
      $element.css('left', outerX === null ? 0 : outerX - this._parentElement().offset().left); ///#DEBUG

      this._testPosX = outerX; ///#ENDDEBUG
    }
  },
  changeCursor: function changeCursor(cursorName) {
    ...
    if ($element) {
      $element.css('cursor', cursorName); ///#DEBUG

      this._testCursorName = cursorName; ///#ENDDEBUG
    }
  },
  dispose: function dispose() {
    this._unsubscribeFromCallback();
...
var DraggingHeaderView = _uiGrid_core.default.View.inherit({
  ...
  dockHeader: function dockHeader(eventData) {
    ...
    if (targetDraggingPanel) {
      ...
      pointsByTarget[targetLocation] = pointsByColumns; ///#DEBUG

      this._testPointsByColumns = pointsByColumns; ///#ENDDEBUG
