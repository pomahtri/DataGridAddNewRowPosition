  ...
  return {
    ...
    _needInsertItem: function _needInsertItem(change, changeType) {
      ...
      if (scrollingMode !== 'standard') {
        ...
        var needInsertOnLastPosition = newRowPosition === _uiGrid_core3.PAGE_BOTTOM_NEW_ROW_POSITION || isLastPage && change.index === -1;
    ...
    _getLoadedRowIndex: function _getLoadedRowIndex(items, e, change) {
      var insertAfterOrBeforeKey = this._getInsertAfterOrBeforeKey(change);

      if ((0, _type.isDefined)(insertAfterOrBeforeKey)) {
        var _loadedRowIndex = _uiGrid_core2.default.getIndexByKey(insertAfterOrBeforeKey, items, this._dataController.key());

        if (_loadedRowIndex < 0) {
          return _loadedRowIndex;
        }

        return (0, _type.isDefined)(change.insertAfterKey) ? _loadedRowIndex + 1 : _loadedRowIndex;
      }
    ...
    processItems: function processItems(items, e) {
      ...
      changes.forEach(function (change) {
        ...
        if (!isInsert || _this8.needChangePageIndexToInsertRow()) {
          return;
        }
        ...
        if (!(0, _type.isDefined)(key) || !(0, _type.isDefined)(insertInfo)) {
          insertInfo = _this8._addInsertInfo(change);
          key = insertInfo.key;
        }

        var loadedRowIndex = _this8._getLoadedRowIndex(items, e, change);

        var insertAfterOrBeforeKey = _this8._getInsertAfterOrBeforeKey(change);

        var item = _this8._generateNewItem(key);

        if (loadedRowIndex >= 0 && (0, _type.isDefined)(insertAfterOrBeforeKey)) {
          items.splice(loadedRowIndex, 0, item);
        } else if ((loadedRowIndex >= 0 || change.index === -1) && _this8._needInsertItem(change, changeType, items, item)) {
          if (change.index !== -1) {
    ...
    _setIndexes: function _setIndexes(change, rowIndex) {
      ...
      switch (newRowPosition) {
        ...
        case _uiGrid_core3.LAST_NEW_ROW_POSITION:
          change.index = -1;
          change.pageIndex = pageCount - 1;
          break;

        case _uiGrid_core3.PAGE_TOP_NEW_ROW_POSITION:
          change.index = scrollingMode === 'standard' ? 0 : pageIndex * pageSize;
          break;

        case _uiGrid_core3.PAGE_BOTTOM_NEW_ROW_POSITION:
          change.index = scrollingMode === 'standard' ? pageSize : pageIndex * pageSize + pageSize;
          break;
    ...
    _getInsertRowIndex: function _getInsertRowIndex(parentKey) {
      ...
      if (rowsView) {
        switch (newRowPosition) {
          ...
          case _uiGrid_core3.PAGE_TOP_NEW_ROW_POSITION:
            return (pageIndex - beginPageIndex) * pageSize;

          case _uiGrid_core3.LAST_NEW_ROW_POSITION:
            return items.length;

          case _uiGrid_core3.PAGE_BOTTOM_NEW_ROW_POSITION:
            return (pageIndex - beginPageIndex) * pageSize + pageSize;

          case _uiGrid_core3.VIEWPORT_BOTTOM_NEW_ROW_POSITION:
            return rowsView.getBottomVisibleItemIndex(true);
    ...
    needChangePageIndexToInsertRow: function needChangePageIndexToInsertRow() {
      var newRowPosition = this.option('editing.newRowPosition');
      var dataController = this._dataController;
      var pageIndex = dataController.pageIndex();
      var pageCount = dataController.pageCount();
      return newRowPosition === _uiGrid_core3.FIRST_NEW_ROW_POSITION && pageIndex !== 0 || newRowPosition === _uiGrid_core3.LAST_NEW_ROW_POSITION && pageIndex !== pageCount - 1;
    },
    changePageIndexToInsertRow: function changePageIndexToInsertRow(insertChange) {
    ...
    _addRowCore: function _addRowCore(data, parentKey, initialOldEditRowIndex) {
      ...
      if (this.needChangePageIndexToInsertRow()) {
        ...
        return this.changePageIndexToInsertRow(change).done(function () {
          var rowIndex = _this12._dataController.getRowIndexByKey(key);

          _this12._showAddedRow(rowIndex);

          _this12._afterInsertRow(key);
        }).fail(d.reject);
      } else {
        var oldEditRowIndex = this._getVisibleEditRowIndex();

        var _this$_addInsertInfo2 = this._addInsertInfo(change, parentKey),
            _key = _this$_addInsertInfo2.key,
            rowIndex = _this$_addInsertInfo2.rowIndex;

        this._setEditRowKey(_key, true);
    ...
    _focusFirstEditableCellInRow: function _focusFirstEditableCellInRow(rowIndex) {
      ...
      this._delayedInputFocus($firstCell, function () {
        _this13._editCellInProgress = false;

        _this13._beforeFocusElementInRow(rowIndex);
      });
    },
    ...
    _getEditPopupHiddenHandler: function _getEditPopupHiddenHandler() {
      ...
      return function (e) {
        if (_this14.isEditing()) {
          _this14.cancelEditData();
        }
      };
    },
    ...
    _delayedInputFocus: function _delayedInputFocus($cell, beforeFocusCallback, callBeforeFocusCallbackAlways) {
      ...
      var inputFocus = function inputFocus() {
        ...
        if ($cell) {
          ...
          _uiGrid_core2.default.focusAndSelectElement(_this16, $focusableElement);
        }

        _this16._beforeFocusCallback = null;
      };
      ...
      if (item && allowDeleting) {
        if (!confirmDelete || !confirmDeleteMessage) {
          ...
          (0, _dialog.confirm)(confirmDeleteMessage, confirmDeleteTitle, showDialogTitle).done(function (confirmResult) {
            if (confirmResult) {
              _this17._deleteRowCore(rowIndex);
            }
          });
        }
      }
    ...
    _fireOnSaving: function _fireOnSaving() {
      ...
      (0, _deferred.when)((0, _deferred.fromPromise)(onSavingParams.promise)).done(function () {
        ...
        _this18._fireDataErrorOccurred(arg);
    ...
    _processChanges: function _processChanges(deferreds, results, dataChanges, changes) {
      ...
      (0, _iterator.each)(changes, function (index, change) {
        var oldData = _this19._getOldData(change.key);
        ...
        if (_this19._beforeSaveEditData(change, index)) {
          return;
        }
        ...
        switch (type) {
          case _uiGrid_core3.DATA_EDIT_DATA_REMOVE_TYPE:
            ...
            deferred = _this19._executeEditingAction('onRowRemoving', params, function () {
          ...
          case _uiGrid_core3.DATA_EDIT_DATA_INSERT_TYPE:
            ...
            deferred = _this19._executeEditingAction('onRowInserting', params, function () {
          ...
          case DATA_EDIT_DATA_UPDATE_TYPE:
            ...
            deferred = _this19._executeEditingAction('onRowUpdating', params, function () {
    ...
    _fireSaveEditDataEvents: function _fireSaveEditDataEvents(changes) {
      ...
      (0, _iterator.each)(changes, function (_, _ref) {
        ...
        var internalData = _this20._addInternalData({
          key: key
        });
        ...
        switch (type) {
          case _uiGrid_core3.DATA_EDIT_DATA_REMOVE_TYPE:
            _this20.executeAction('onRowRemoved', (0, _extend.extend)({}, params, {
          ...
          case _uiGrid_core3.DATA_EDIT_DATA_INSERT_TYPE:
            _this20.executeAction('onRowInserted', params);

            break;

          case DATA_EDIT_DATA_UPDATE_TYPE:
            _this20.executeAction('onRowUpdated', params);
    ...
    saveEditData: function saveEditData() {
      ...
      this.waitForDeferredOperations().done(function () {
        if (_this21.isSaving()) {
          _this21._resolveAfterSave(deferred);

          return;
        }

        (0, _deferred.when)(_this21._beforeSaveEditData()).done(function (cancel) {
          if (cancel) {
            _this21._resolveAfterSave(deferred, {
              cancel: cancel
            });
          ...
          _this21._saving = true;

          _this21._saveEditDataInner().always(function () {
            _this21._saving = false;
    ...
    _saveEditDataInner: function _saveEditDataInner() {
      ...
      (0, _deferred.when)(this._fireOnSaving()).done(function (_ref3) {
        ...
        _this22._processChanges(deferreds, results, dataChanges, changes);

        if (deferreds.length) {
          dataSource === null || dataSource === void 0 ? void 0 : dataSource.beginLoading();

          _deferred.when.apply(void 0, deferreds).done(function () {
            if (_this22._processSaveEditDataResult(results)) {
              _this22._endSaving(dataChanges, changes, result);
            } else {
          ...
          return result.always(function () {
            _this22._focusEditingCell();
          }).promise();
        }

        _this22._cancelSaving(result);
      }).fail(result.reject);
    ...
    _refreshDataAfterSave: function _refreshDataAfterSave(dataChanges, changes, deferred) {
      ...
      (0, _deferred.when)(dataController.refresh({
        ...
        _this23._fireSaveEditDataEvents(changes);
      }).done(function () {
    ...
    addDeferred: function addDeferred(deferred) {
      ...
      if (this._deferreds.indexOf(deferred) < 0) {
        ...
        deferred.always(function () {
          var index = _this24._deferreds.indexOf(deferred);

          if (index >= 0) {
            _this24._deferreds.splice(index, 1);
          }
        });
      }
    },
    _prepareChange: function _prepareChange(options, value, text) {
      ...
      if (rowKey !== undefined) {
        ...
        setCellValueResult.done(function () {
          ...
          return _this25._fireDataErrorOccurred(arg);
        });
    ...
    updateFieldValue: function updateFieldValue(options, value, text, forceUpdateRow) {
      ...
      if (options.column.setCellValue) {
        this._prepareChange(options, value, text).done(function (params) {
          (0, _deferred.when)(_this26._applyChange(options, params, forceUpdateRow)).always(function () {
            deferred.resolve();
          });
        });
      } else {
    ...
    _updateRowWithDelay: function _updateRowWithDelay(row, isCustomSetCellValue) {
      ...
      setTimeout(function () {
        ...
        var columnIndex = _this27._rowsView.getCellIndex($focusedElement, row.rowIndex);

        var focusedElement = $focusedElement.get(0);

        var selectionRange = _uiGrid_core2.default.getSelectionRange(focusedElement);

        _this27._updateEditRowCore(row, false, isCustomSetCellValue);

        _this27._validateEditFormAfterUpdate(row, isCustomSetCellValue);

        if (columnIndex >= 0) {
          var $focusedItem = _this27._rowsView._getCellElement(row.rowIndex, columnIndex);

          _this27._delayedInputFocus($focusedItem, function () {
            setTimeout(function () {
        ...
        if (((allowUpdating || isRowEditing) && column.allowEditing || isCellEditing) && (isRowEditing || !isRowMode)) {
          ...
          if (!editingStartOptions || !editingStartOptions.cancel) {
            options.setValue = function (value, text) {
              _this28.updateFieldValue(options, value, text);
            };
          }
        }
    ...
    prepareButtonItem: function prepareButtonItem(headerPanel, name, methodName, sortIndex) {
      ...
      var onInitialized = function onInitialized(e) {
        (0, _renderer.default)(e.element).addClass(headerPanel._getToolbarButtonClass(EDIT_BUTTON_CLASS + ' ' + _this29.addWidgetPrefix(className) + '-button'));
      };
    ...
    controllers: {
      data: {
        ...
        _fireChanged: function _fireChanged(e) {
          ...
          var insertChanges = changes.filter(function (change) {
            return change.type === 'insert';
          });
          var unprocessedInsertChanges = insertChanges.filter(function (change) {
            var _editingController$_g;

            return !((_editingController$_g = editingController._getInternalData(change.key)) !== null && _editingController$_g !== void 0 && _editingController$_g.insertInfo);
          });
          var lastInsertChange = unprocessedInsertChanges.length && unprocessedInsertChanges[insertChanges.length - 1];
          this.callBase(e);
      ...
      rowsView: {
        ...
        _rowPointerDown: function _rowPointerDown(e) {
          ...
          this._pointerDownTimeout = setTimeout(function () {
            _this30._editCellByClick(e, 'down');
          });
        },
