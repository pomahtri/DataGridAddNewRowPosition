  ...
  return {
    ...
    _needInsertItem: function _needInsertItem(change, changeType) {
      ...
      if (scrollingMode !== 'standard') {
        ...
        var needInsertOnLastPosition = newRowPosition === PAGE_BOTTOM_NEW_ROW_POSITION || isLastPage && change.index === -1;
    ...
    _getLoadedRowIndex: function _getLoadedRowIndex(items, e, change) {
      ...
      if (isDefined(insertAfterOrBeforeKey)) {
        var _loadedRowIndex = gridCoreUtils.getIndexByKey(insertAfterOrBeforeKey, items, this._dataController.key());

        if (_loadedRowIndex < 0) {
          return _loadedRowIndex;
        }

        return isDefined(change.insertAfterKey) ? _loadedRowIndex + 1 : _loadedRowIndex;
      }
      ...
      var changes = this.getChanges();
      var index = e ? changes.filter(editChange => equalByValue(editChange.key, key))[0].index : 0;
    ...
    processItems: function processItems(items, e) {
      ...
      changes.forEach(change => {
        ...
        var loadedRowIndex = this._getLoadedRowIndex(items, e, change);

        var insertAfterOrBeforeKey = this._getInsertAfterOrBeforeKey(change);

        var item = this._generateNewItem(key);

        if (loadedRowIndex >= 0 && isDefined(insertAfterOrBeforeKey)) {
          items.splice(loadedRowIndex, 0, item);
        } else if ((loadedRowIndex >= 0 || change.index === -1) && this._needInsertItem(change, changeType, items, item)) {
          if (change.index !== -1) {
    ...
    _getInsertRowIndex: function _getInsertRowIndex(parentKey) {
      ...
      if (rowsView) {
        switch (newRowPosition) {
          ...
          case VIEWPORT_BOTTOM_NEW_ROW_POSITION:
    ...
    controllers: {
      data: {
        ...
        _fireChanged: function _fireChanged(e) {
          ...
          var insertChanges = changes.filter(change => change.type === 'insert');
          var unprocessedInsertChanges = insertChanges.filter(change => {
