...
var SchedulerAppointments = /*#__PURE__*/function (_CollectionWidget) {
  ...
  _proto._resizeEndHandler = function _resizeEndHandler(e) {
    ...
    var modifiedAppointmentAdapter = (0, _appointmentAdapter.createAppointmentAdapter)(this.option('key'), sourceAppointment).clone();
  ...
  _proto._getEndResizeAppointmentStartDate = function _getEndResizeAppointmentStartDate(e, rawAppointment, appointmentInfo) {
    var appointmentAdapter = (0, _appointmentAdapter.createAppointmentAdapter)(this.option('key'), rawAppointment);
  ...
  _proto._sortAppointmentsByStartDate = function _sortAppointmentsByStartDate(appointments) {
    (0, _instanceFactory.getAppointmentDataProvider)(this.option('key')).sortAppointmentsByStartDate(appointments);
  };
  ...
  _proto._processRecurrenceAppointment = function _processRecurrenceAppointment(appointment, index, skipLongAppointments) {
    ...
    var recurrenceRule = _expressionUtils.ExpressionUtils.getField(this.option('key'), 'recurrenceRule', appointment);
    ...
    if (recurrenceRule) {
      ...
      var startDate = new Date(_expressionUtils.ExpressionUtils.getField(this.option('key'), 'startDate', dates));
      var endDate = new Date(_expressionUtils.ExpressionUtils.getField(this.option('key'), 'endDate', dates));
      var appointmentDuration = endDate.getTime() - startDate.getTime();

      var recurrenceException = _expressionUtils.ExpressionUtils.getField(this.option('key'), 'recurrenceException', appointment);
  ...
  _proto._processLongAppointment = function _processLongAppointment(appointment, result) {
    ...
    if (partCount > 1) {
      ...
      for (var i = 1; i < partCount; i++) {
        var startDate = _expressionUtils.ExpressionUtils.getField(this.option('key'), 'startDate', parts[i].settings).getTime();
  ...
  _proto._applyStartDateToObj = function _applyStartDateToObj(startDate, obj) {
    _expressionUtils.ExpressionUtils.setField(this.option('key'), 'startDate', obj, startDate);

    return obj;
  };

  _proto._applyEndDateToObj = function _applyEndDateToObj(endDate, obj) {
    _expressionUtils.ExpressionUtils.setField(this.option('key'), 'endDate', obj, endDate);
  ...
  _proto.splitAppointmentByDay = function splitAppointmentByDay(appointment) {
    var dates = appointment.settings || appointment;
    var originalStartDate = new Date(_expressionUtils.ExpressionUtils.getField(this.option('key'), 'startDate', dates));

    var startDate = _date.default.makeDate(originalStartDate);

    var endDate = _date.default.makeDate(_expressionUtils.ExpressionUtils.getField(this.option('key'), 'endDate', dates));
