...
function getCloudAngle(_ref, x, y, anchorX, anchorY) {
  ...
  if (anchorX < xl && anchorY < yt || anchorX >= xl && anchorX <= xr && anchorY < yt // 2
  ) {
    return 270;
  }

  if (anchorX > xr && anchorY > yb || anchorX >= xl && anchorX <= xr && anchorY > yb // 6
  ) {
    return 90;
  } else if (anchorX < xl && anchorY > yb || anchorX < xl && anchorY >= yt && anchorY <= yb // 8
  ) {
    return 180;
  }
