...
if (Quill) {
  ...
  var getTargetTableNode = (quill, partName) => {
    var [table, cell] = quill.getModule('table').getTable();
  ...
  ToolbarModule = class ToolbarModule extends BaseModule {
    ...
    _getFormatHandlers() {
      return {
        ...
        cellProperties: () => {
          var domNode = getTargetTableNode(this.quill, 'cell');
          showCellPropertiesForm(this.editorInstance, $(domNode));
        },
        tableProperties: () => {
          var domNode = getTargetTableNode(this.quill, 'table');
