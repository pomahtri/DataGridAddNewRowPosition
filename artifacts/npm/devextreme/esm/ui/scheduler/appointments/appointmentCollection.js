...
class SchedulerAppointments extends CollectionWidget {
  ...
  _resizeEndHandler(e) {
    ...
    var modifiedAppointmentAdapter = createAppointmentAdapter(this.option('key'), sourceAppointment).clone();
  ...
  _getEndResizeAppointmentStartDate(e, rawAppointment, appointmentInfo) {
    var appointmentAdapter = createAppointmentAdapter(this.option('key'), rawAppointment);
  ...
  _processRecurrenceAppointment(appointment, index, skipLongAppointments) {
    ...
    var recurrenceRule = ExpressionUtils.getField(this.option('key'), 'recurrenceRule', appointment);
    var result = {
      parts: [],
      indexes: []
    };
    ...
    if (recurrenceRule) {
      ...
      var startDate = new Date(ExpressionUtils.getField(this.option('key'), 'startDate', dates));
      var endDate = new Date(ExpressionUtils.getField(this.option('key'), 'endDate', dates));
      var appointmentDuration = endDate.getTime() - startDate.getTime();
      var recurrenceException = ExpressionUtils.getField(this.option('key'), 'recurrenceException', appointment);
  ...
  _processLongAppointment(appointment, result) {
    ...
    if (partCount > 1) {
      ...
      for (var i = 1; i < partCount; i++) {
        var startDate = ExpressionUtils.getField(this.option('key'), 'startDate', parts[i].settings).getTime();
  ...
  splitAppointmentByDay(appointment) {
    var dates = appointment.settings || appointment;
    var originalStartDate = new Date(ExpressionUtils.getField(this.option('key'), 'startDate', dates));
    var startDate = dateUtils.makeDate(originalStartDate);
    var endDate = dateUtils.makeDate(ExpressionUtils.getField(this.option('key'), 'endDate', dates));
